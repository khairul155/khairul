...
serve(async (req) => {
  ...
  try {
    ...
    if (createError) {
      console.error("Error creating table with SQL RPC:", createError.message);
      
      // Try alternative method using SQL API if RPC fails
      const { error: sqlError } = await supabaseAdmin.from('_sql').select('*').execute(`
        CREATE TABLE IF NOT EXISTS public.user_credits (
          id SERIAL PRIMARY KEY,
          user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
          subscription_plan TEXT NOT NULL DEFAULT 'free',
      ...
      if (sqlError) {
        console.error("Error creating table with SQL API:", sqlError.message);
        throw sqlError;
      }
    }
