  ...
  try {
    ...
    if (action === "deduct") {
      ...
      if (userData.subscription_plan === 'free') {
        ...
        // For paid plans, use monthly credits or allow unlimited
        if (userData.monthly_credits > 0) {
          ...
          return new Response(
            JSON.stringify({ 
              credits: 999999, // Indicate unlimited
              deducted: 0, // Not deducting from a limited pool
              status: 'success'
            }),
            { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
    ...
    if (userError || !userData) {
      // If not found, assume they are on the free plan with 5 tokens
      return new Response(
        JSON.stringify({ 
          credits: 5,
    ...
    if (userData.subscription_plan === 'free') {
      ...
      // For paid plan users - return monthly credits or unlimited
      if (userData.monthly_credits > 0) {
        return new Response(
          JSON.stringify({ 
            credits: userData.monthly_credits - userData.credits_used_this_month,
            resetDate: userData.next_reset_date,
            plan: userData.subscription_plan
          }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
